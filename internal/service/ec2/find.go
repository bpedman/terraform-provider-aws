...
func FindRouteByIPv4Destination(ctx context.Context, conn *ec2.EC2, routeTableID, destinationCidr string) (*ec2.Route, error) {
	...
	for _, route := range routeTable.Routes {
		if types.CIDRBlocksEqual(aws.StringValue(route.DestinationCidrBlock), destinationCidr) && aws.StringValue(route.Origin) != ec2.RouteOriginEnableVgwRoutePropagation {
			return route, nil
		}
	}
